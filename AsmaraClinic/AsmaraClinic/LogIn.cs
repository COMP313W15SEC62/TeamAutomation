    ...
    public partial class LogIn : Form
    {
        ...
        private void btnNewAccount_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                ("Insert LoginInfo (Password, StaffID, UserName, Role) Values ('{0}', '{1}', '{2}', '{3}' )",
                hashedPassword, int.Parse(txtID.Text.Trim()), txtUserName.Text.Trim(), cmbRole.Text.Trim());
        ...
        private void btnLigIn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cn.ConnectionString = MyConnString;
                string sqlStr =
                string.Format("Select * from LoginInfo Where UserName='{0}'",
                ...
                string storedPwd = "";
                int storedRole;
                string storedUerName;
                
                while (myReader.Read())
                {
                    storedPwd = myReader["Password"].ToString().Trim();
                    storedUerName = myReader.GetString(2);
                    storedRole = myReader.GetInt32(3);
                   ...
                   // myReader.Close();
                    ...
                    else if(storedUerName != this.txtUserName.Text.Trim())
                    {
                        MessageBox.Show("Sorry, your userame is not correct");
                    }
                    //else if (storedUerName != this.txtUserName.Text.Trim() && !passWordIsValid)
                    //{
                    //    MessageBox.Show("Sorry, your userame and password are not correct");
                    //}
                    //else if (storedUerName != this.txtUserName.Text.Trim() || !passWordIsValid)
                    //{
                    //    MessageBox.Show("Sorry, your userame and password are not correct");
                    //}
                    else if (passWordIsValid && storedRole == 2)
                    {

                        MessageBox.Show("Userame and password is correct");
                        this.Hide();
                        Receptionist f2 = new Receptionist(txtUserName.Text);
                        f2.ShowDialog();
                    }

                    else if (passWordIsValid && storedRole == 1)
                    {
                        MessageBox.Show("Userame and password is correct");
                        this.Hide();
                        Doctor f3 = new Doctor(txtUserName.Text);
                        f3.ShowDialog();
                    }
        ...
        {
            lblID.Show(); lblRole.Show();
            txtID.Show(); cmbRole.Show();
        }
    }
}
